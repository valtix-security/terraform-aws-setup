...
resource "aws_s3_bucket_notification" "valtix_s3_bucket_notification" {
  ...
  bucket   = aws_s3_bucket.valtix_s3_bucket[each.key].id
  queue {
    queue_arn = "arn:aws:sqs:${data.aws_region.current.region}:${var.controller_aws_account_number}:inventory_logs_queue_${var.deployment_name}_${data.aws_region.current.region}"
